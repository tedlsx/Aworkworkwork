...
class Text(object):
    ...
    def nouns_more_common_in_plural_form(self):
        plural_list = [i[0] for i in self.word_tag if i[1] == 'NNS' and i[0][0].isalpha()]
        plural_freq = FreqDist(plural_list)
        single_list = [i[0] for i in self.word_tag if i[1] == 'NN' and i[0][0].isalpha()]
        single_freq = FreqDist(single_list)
        word_list = set(plural_list)
        out_list = []
        for word in word_list:
            single_form = word[:-1]
            if plural_freq[word] > single_freq[single_form] and single_freq[single_form] != 0:
                out_list.append(single_form)
...
if __name__ == "__main__":
    ...
    if len(take_arg) == 2:
        method = take_arg[1]
        if 'train' in method:
            training_model = postag.train_model(news)
        else:
        ...
        if 'run' in method and isinstance(argv, str):
            sents = argv.split(' ')
            postag.run(training_model, sents)
        elif 'test' in method:
            data_tag = brown.tagged_sents(categories = argv)
            postag.test(training_model, data_tag)
